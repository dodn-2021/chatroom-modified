...
public class MainViewController implements Initializable
{
    ...
    @FXML
    private ListView< UserCard > friendListView;
    @FXML
    private ListView< UserCard > groupListView;
    @FXML
    ...
    {
        StageManager.getManager().resetStage(Resource.AddFriendView, new AddFriendView());
        StageManager.getManager().show(Resource.AddFriendView);
    }
    ...
    public void createGroup() throws IOException
    {
        ...
        File iconFile = UiUtilities.showFileChooser("group icon", "jpg"
                , "jpeg", "bmp", "png");
        if( iconFile == null ){
            return;
        }
        ...
        if( CommunicationUtilities.createGroup(rs.get(), bytes) ){
    ...
    public void initialize( URL location, ResourceBundle resources )
    {
        for( Friend friend : User.getInstance().getFriendList() ){
            UserCard card = friend.getUserEntity().getUserCard();
        ...
        User.getInstance().getFriends().addListener(( obs, ov, nv ) -> {
            ...
            for( Friend friend : nv.values() ){
                if( friend.getUserEntity().getUserCard() == null ){
                    friend.getUserEntity().prepareUserCard();
                }
                friendListView.getItems().add(friend.getUserEntity().getUserCard());
            }
        });
